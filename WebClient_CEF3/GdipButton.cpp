	...
	if(!m_bHaveBitmaps)
	{
		...
		if (m_dcStd.m_hDC == NULL)
		{
			...
			if (m_dcStdP.m_hDC == NULL)
			{
				...
				graphics.DrawImage(*m_pStdImage, 1, 1, rect.Width(), rect.Height());
			...
			if(m_dcStdH.m_hDC == NULL)
			{
				...
				int w = m_pStdImage->m_pBitmap->GetWidth();
				int h = m_pStdImage->m_pBitmap->GetHeight();
			...
			//	graphics.DrawImage(*m_pStdImage, grect, 0, 0, rect.Width(), rect.Height(), UnitPixel, &ia);
		//		graphics.DrawImage(*m_pStdImage, 1, 1, rect.Width(), rect.Height());
